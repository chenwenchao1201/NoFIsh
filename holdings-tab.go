...
func (app *Config) holdingsTab() *fyne.Container {
	...
	holdingsContainer := container.NewBorder(nil, nil, nil, nil, container.NewAdaptiveGrid(1, app.HoldingsTable))
		...
		func(i widget.TableCellID, o fyne.CanvasObject) {
			if i.Col == (len(app.Holdings[0])-1) && i.Row != 0 {
				...
				w := widget.NewButtonWithIcon("Delete", theme.DeleteIcon(), func() {
					...
					dialog.ShowConfirm("Delete Holding", "Are you sure you want to delete this holding?", func(delete bool) {
				...
				// we are puttig text
				o.(*fyne.Container).Objects = []fyne.CanvasObject{widget.NewLabel(app.Holdings[i.Row][i.Col].(string))}
...
func (app *Config) getHoldingSlice() [][]interface{} {
	...
	for _, x := range holdings {
		...
		currentRow = append(currentRow, widget.NewButtonWithIcon("Delete", theme.DeleteIcon(), func() {}))
